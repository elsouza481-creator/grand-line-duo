...
object CombatModifierResolver {
    fun forWorld(world: WorldState): Map<String, CombatModifiers> = world.players.mapValues { (playerId, player) ->
        ...
        val classBonus = ClassMasteryCombatResolver.resolve(profile?.classMastery)
        ...
        CombatModifiers(
            attackBonus = item.attackDamage + classBonus.attackBonus + (buso * 2) + hao + fruitAttack,
            damageReduction = item.damageReduction + classBonus.damageReduction + buso + ((ken + 1) / 2),
